...
pub const NSTextLayoutOrientationProviderMessages = struct {
    ...
    pub fn initClass(_class_name: [:0]const u8) objc.Class {
        ...
        if (class == null) {
            class = runtime_support.backend_support.ObjectRegistry.newDelegateClass(_class_name, "NSTextLayoutOrientationProvider");
        }
        return class.?;
    }
};
