...
pub const NSUserInterfaceValidationsMessages = struct {
    ...
    pub fn initClass(_class_name: [:0]const u8) objc.Class {
        ...
        if (class == null) {
            class = runtime_support.backend_support.ObjectRegistry.newDelegateClass(_class_name, "NSUserInterfaceValidations");
        }
        return class.?;
    }
};
...
pub const NSValidatedUserInterfaceItemMessages = struct {
    ...
    pub fn initClass(_class_name: [:0]const u8) objc.Class {
        var class = objc.getClass(_class_name);
        if (class == null) {
            class = runtime_support.backend_support.ObjectRegistry.newDelegateClass(_class_name, "NSValidatedUserInterfaceItem");
        }
        return class.?;
    }
};
